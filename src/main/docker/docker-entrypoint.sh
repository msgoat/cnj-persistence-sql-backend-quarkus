#!/bin/sh

if [ "$1" = "start" ]
then
	echo "Running Quarkus application with options:"
	echo "JAVA_APPLICATION=`ls ${JAVA_APPLICATION_HOME}/*-runner.jar`"
	echo "JAVA_APPLICATION_HOME=${JAVA_APPLICATION_HOME}"
	echo "JAVA_OPTS=${JAVA_OPTS}"
	echo "EXT_JAVA_OPTS=${EXT_JAVA_OPTS}"
	echo "DOCKER_JAVA_OPTS=${DOCKER_JAVA_OPTS}"
	echo "AGENT_JAVA_OPTS=${AGENT_JAVA_OPTS}"
	echo "QUARKUS_JAVA_OPTS=${QUARKUS_JAVA_OPTS}"
	echo "QUARKUS_ARGUMENTS=${QUARKUS_ARGUMENTS}"
	java ${JAVA_OPTS} ${EXT_JAVA_OPTS} ${DOCKER_JAVA_OPTS} ${AGENT_JAVA_OPTS} ${QUARKUS_JAVA_OPTS} -jar ${JAVA_APPLICATION_HOME}/*-runner.jar ${QUARKUS_ARGUMENTS}
else
	exec "$@"	
fi
